    ...
    if (arguments.length == 0) {
        ...
        c.onload = function () {
            ...
            if (f != null) {
                ...
                /*window.EXIF.getData(d, function () {
                    var orientation = EXIF.getTag(this, "Orientation");
                    var canvas = window.loadImage.scale(d, {orientation: orientation || 0, canvas: true});
                    alert(orientation);
                ...
                if ([5,6,7,8].indexOf(srcOrientation) > -1) {
                    f.width = height;
                    f.style.width = height;
                    f.height = width;
                    f.style.height = width;
                }

                // transform context before drawing image
                switch (srcOrientation) {
                  case 2: g.transform(-1, 0, 0, 1, width, 0); break;
                  case 3: g.transform(-1, 0, 0, -1, width, height ); break;
                  case 4: g.transform(1, 0, 0, -1, 0, height ); break;
                  case 5: g.transform(0, 1, 1, 0, 0, 0); break;
                  case 6: g.transform(0, 1, -1, 0, height , 0); break;
                  case 7: g.transform(0, -1, -1, 0, height , width); break;
                  case 8: g.transform(0, -1, 1, 0, 0, width); break;
            ...
            try {
                ...
                qrcode.result = "Couldn't resolve the QR code image, try again or enter the code manually."
            }
            if (qrcode.callback != null) {
                qrcode.callback(qrcode.result)
            }
...
function _x48() {
    ...
    this._x7a = function () {
        ...
        if (a < 3) {
            console.log("Couldn't find enough finder patterns.");
            throw "Couldn't resolve the QR code image, try again or enter the code manually.";
        }
        if (a > 3) {
            var b = 0;
            for (var c = 0; c < a; c++) {
...
function _x1a(g, c, b, f, a, e, d) {
    ...
    this.find = function () {
        ...
        console.log("Couldn't find enough alignment patterns");
        throw "Couldn't resolve the QR code image, try again or enter the code manually.";
    }
}
